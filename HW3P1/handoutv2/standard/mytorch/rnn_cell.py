...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        # self.W_ih =  (hidden_size, input_size)
        #d = self.input_size
        input = np.dot(x, self.W_ih.T) + self.b_ih
        hidden = np.dot(h_prev_t, self.W_hh.T) + self.b_hh
        h_t = self.activation.forward(input + hidden)
        return h_t
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        dz = delta * self.activation.backward(dLdA=delta, state=h_t)
